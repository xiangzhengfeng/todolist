...
interface ItemProps {
  item: listItemProps
  index: number
  handleClick: (is_done: number, id: number) => void
}
...
export default function Item(props: ItemProps) {
  ...
  return (
    <SwipeAction
      ...
      right={[
        {
          text: item.isDone === 1 ? '删除' : '完成',
          onPress: () => {
            alert('提示', '确定进行此操作？', [
              { text: '取消', onPress: () => console.log('cancel'), style: 'default' },
              {
                text: '确定', onPress: () => {
                  handleClick(item.isDone, item.id)
                }
              },
            ]);
          },
          style: { backgroundColor: item.isDone === 1 ? 'red' : 'green', color: 'white' },
        },
      ]}
    >
      <List.Item
      ...
      >
        {item.isDone === 1 ? <TextDone>{index + 1 + '. ' + item.value}</TextDone> : <TextNoDo>{index + 1 + '. ' + item.value}</TextNoDo>}
      </List.Item>
    </SwipeAction>
  )
}