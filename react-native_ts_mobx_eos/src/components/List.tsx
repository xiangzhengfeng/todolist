...
@inject('ListStores')
@observer
export default class List extends React.Component<{ ListStores?: ListStore }> {
  state = {
    type: 0
  }

  componentWillMount(){
    this.props.ListStores.readListStore()
  }

  render() {
    const { list, tabList } = this.props.ListStores
        ...
        {['全部事项', '未完成', '已完成'].map((item, i) => {
          return (
            <TouchableOpacity onPress={() => {
              ...
              this.props.ListStores.getTabList(i)
            }}>
            ...
            ? <SwipeListView
              ...
              renderHiddenItem={(data: any, rowMap: any) => {
                return (<HiddenBtn
                  ...
                  rowMap={rowMap}
                  list={tabList}
                  handleClick={key => this.props.ListStores.handleDone(key)} />)
              }}
              rightOpenValue={-60}
              previewRowKey={'0'}
              previewOpenValue={-40}
